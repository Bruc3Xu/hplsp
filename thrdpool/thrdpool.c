...
inline void __thrdpool_schedule(const struct Task *task, void *buf,
                                thrdpool_t *pool);

void __thrdpool_schedule(const struct Task *task, void *buf, thrdpool_t *pool) {
  ((struct __thrdpool_task_entry *)buf)->task = *task;
  msgqueue_put(buf, pool->msgqueue);
}

int thrdpool_schedule(const struct Task *task, thrdpool_t *pool) {
...
void thrdpool_destroy(void (*pending)(const struct Task *), thrdpool_t *pool) {
