...
Logger::Logger() : shutdown_(false), level_(0), dest_(0) { _Reset(); }

Logger::~Logger() { _CloseLogFile(); }
...
const std::string &Logger::_MakeFileName() {
  ...
  seq_++;
  fileName_ =
      directory_ + "/" + name + pid.str() + std::to_string(seq_) + ".log";
...
void Logger::_CloseLogFile() { return file_.Close(); }
...
void Logger::Flush(enum LogLevel level) {
  ...
  switch (level) {
  case logINFO:
    memcpy(tmpBuffer_ + kPrefixTimeLen, "[INFO]:", kPrefixLevelLen);
    break;

  case logDEBUG:
    memcpy(tmpBuffer_ + kPrefixTimeLen, "[DBGUG]:", kPrefixLevelLen);
    break;

  case logWARN:
    memcpy(tmpBuffer_ + kPrefixTimeLen, "[WARN]:", kPrefixLevelLen);
    break;

  case logERROR:
    memcpy(tmpBuffer_ + kPrefixTimeLen, "[ERROR]:", kPrefixLevelLen);
    break;
...
Logger &Logger::operator<<(void *ptr) {
  ...
  if (pos_ + 18 < kMaxCharPerLog) {
    ...
    auto nbytes =
        snprintf(tmpBuffer_ + pos_, kMaxCharPerLog - pos_, "%#018lx", ptrValue);
    if (nbytes > 0)
      pos_ += nbytes;
  }
...
LogManager::LogManager() : shutdown_(true) { nullLog_.Init(0); }
